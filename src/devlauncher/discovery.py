...
def _infer_backend(score: _DirScore) -> Tuple[str, int, List[str]]:
    ...
    if fw == "fastapi":
        ...
        # 1. Flat files in the service directory
        for candidate in ("main.py", "app.py", "server.py", "run.py"):
            if (directory / candidate).exists():
                entry = f"{candidate[:-3]}:app"
                break

        # 2. Common package layouts: app/main.py, src/main.py, etc.
        if entry is None:
            for pkg in ("app", "src", "api", "server"):
                pkg_dir = directory / pkg
                if not pkg_dir.is_dir():
                    continue
                for candidate in ("main.py", "app.py", "server.py"):
                    if (pkg_dir / candidate).exists():
                        entry = f"{pkg}.{candidate[:-3]}:app"
                        break
                if entry is None and (pkg_dir / "__init__.py").exists():
                    entry = f"{pkg}:app"
                if entry:
                    break
        ...
        if entry is None:
            warnings.append(
                "Could not find FastAPI entry point — defaulting to 'main:app'. "
                "Update dev.toml if wrong."
            )
            entry = "main:app"
